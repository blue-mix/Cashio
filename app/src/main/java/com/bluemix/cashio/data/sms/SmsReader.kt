...
class SmsReader(private val context: Context) {
    ...
    private fun performIncrementalSync(): List<ParsedSmsTransaction> {
        ...
        // We look a bit further back (1 hour buffer) just in case of out-of-order delivery
        val safeTimestamp = if (lastSyncTimestamp > 0) lastSyncTimestamp - 3600000 else 0

        val newTransactions = readRecentSms(limit = 200, sinceTimestamp = safeTimestamp)
...
//    }
    ...
    private fun readRecentSms(
        ...
        try {
            ...
            val selectionArgs =
                if (sinceTimestamp != null) arrayOf(sinceTimestamp.toString()) else null
            ...
            context.contentResolver.query(uri, projection, selection, selectionArgs, sortOrder)
                ?.use { cursor ->
                    ...
                    while (cursor.moveToNext()) {
                        if (bodyIdx == -1 || dateIdx == -1) continue
                        ...
                        if (SmsParser.isBankSms(body)) {
                            SmsParser.parseSms(body, dateTime)?.let { parsedTransactions.add(it) }
                        }
                    }
                }
        } catch (e: Exception) {
