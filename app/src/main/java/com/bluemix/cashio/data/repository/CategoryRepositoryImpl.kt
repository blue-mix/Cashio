...
class CategoryRepositoryImpl(
    ...
    override suspend fun seedDefaults(): Result<Boolean> = withContext(Dispatchers.IO) {
        resultOf {
            ...
            DatabaseSeeder.seedDefaultCategories(realmManager.realm)
        }
    }

    override fun observeCategories(): Flow<List<Category>> {
        return realm.query<CategoryEntity>()
            .sort("name", Sort.ASCENDING) // ✅ Consistent alphabetical order
            .asFlow()
    ...
    override suspend fun addCategory(category: Category): Result<Unit> =
        withContext(Dispatchers.IO) {
            resultOf {
                realm.write { copyToRealm(category.toEntity()) }
                Unit
            }
        }
    ...
    override suspend fun isCategoryInUse(categoryId: String): Result<Boolean> =
        withContext(Dispatchers.IO) {
            resultOf {
                ...
                val count =
                    realm.query<ExpenseEntity>("categoryId == $0", categoryId).count().find()
                count > 0
            }
        }
}