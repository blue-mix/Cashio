...
object CashioSemantic {
    ...
    val ExpenseRedDark = Color(0xFFF87171)
...
val md_theme_light_outlineVariant = CashioNeutral.GrayLight
...
/* -------------------------------------------------------------------------- */
/* Convenience extension — converts a domain colorHex Long to Compose Color.  */
/* Called ONLY at the UI/Compose boundary, never in ViewModel or domain.      */
/* -------------------------------------------------------------------------- */

/**
 * Converts an ARGB [Long] stored in domain models (e.g. [Category.colorHex])
 * to a Compose [Color].
 *
 * Usage (UI layer only):
 * ```
 * val composeColor = category.colorHex.toComposeColor()
 * ```
 */
fun Long.toComposeColor(): Color = Color(this.toUInt().toLong())